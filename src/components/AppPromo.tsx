...
function CalendarIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 48 48" fill="none" strokeWidth="1.2" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round">
      ...
      <path d="M32 6L32 14" />
      <path d="M8 18L40 18" />
      <circle cx="24" cy="26" r="2" opacity="0.5" />
      <circle cx="18" cy="34" r="2" opacity="0.3" />
    </svg>
  );
}
...
export default function AppPromo() {
  return (
    <section className="relative bg-white py-24 md:py-32 overflow-hidden">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(196,137,138,0.05),transparent_70%)]" />

      {/* Floating decorations */}
      <svg className="absolute top-16 left-[8%] opacity-[0.06] animate-float" width="30" height="30" viewBox="0 0 30 30" fill="none">
        <path d="M15 1L18 13L30 15L18 17L15 29L12 17L0 15L12 13Z" fill="var(--color-rose-gold)" />
      </svg>
      <svg className="absolute bottom-20 right-[10%] opacity-[0.06] animate-float-reverse" width="24" height="24" viewBox="0 0 24 24" fill="none">
      ...
      <div className="relative mx-auto max-w-6xl px-6">
        ...
        <div className="mx-auto max-w-3xl">
            ...
            {[
              ...
              <div key={step} className="group text-center">
                <div className="relative mx-auto mb-5">
                  <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-2xl border border-rose-gold/15 transition-all duration-300 group-hover:bg-rose-gold group-hover:border-rose-gold">
                    <Icon className="h-10 w-10 text-rose-gold transition-colors group-hover:text-white" />
                  </div>
                  <span className="absolute -top-2 -right-2 flex h-7 w-7 items-center justify-center rounded-full bg-rose-gold text-xs font-medium text-white">
                    {step}
                  </span>
                </div>
                <p className="text-lg font-heading font-semibold text-charcoal">{title}</p>
          ...
          <div className="mt-14 flex flex-col items-center justify-center gap-4 sm:flex-row">
            <a
              href="#"
              className="inline-flex items-center gap-3 rounded-full bg-charcoal px-8 py-4 font-medium text-white transition-all duration-300 hover:bg-medium-gray hover:scale-105 hover:shadow-lg"
            >
